...
#' @param prl Logical, whether to use parallel sampling. Default to \code{prl = FALSE} for not parallelizing. If \code{prl = TRUE}, \strong{\emph{one must first register clusters by calling \code{registerDoSNOW()} externally}}.
#'
#' @return A list of lists, with length \code{reps * m * q}, where \code{m} is the number of different convergence rates \code{m = length(cn)}. Every sub-list has the following format.
#' \itemize{
