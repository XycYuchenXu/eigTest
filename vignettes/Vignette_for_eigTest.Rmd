...
eg_full_mat = full_samples[[1]]$mu.bar; print(dim(eg_full_mat))
eg_full_cov = full_samples[[1]]$cov.bar; print(dim(eg_full_cov))
...
eg_part_mat = part_samples[[1]]$mu.bar; print(dim(eg_part_mat))
eg_part_cov = part_samples[[1]]$cov.bar; print(dim(eg_part_cov))
```
...
commutatorTest(mat.arr = eg_full_mat,              # sample matrices
               cn = full_samples[[1]]$CovRate,     # convergence rate
               cov.arr = eg_full_cov,              # covariance matrices
               testType = 'chi',                   # type of test statistic
               param.out = TRUE)$chi               # return parameters

## Log-likelihood ratio test
projTest(A = eg_part_mat,                          # sample matrices
         cn = part_samples[[1]]$CovRate,           # convergence rate
         cov.arr = eg_part_cov,                    # covariance matrices
...
V = JDTE(eg_full_mat); V
```

### Multi-sample test

```{r multiSample, echo=TRUE, message=FALSE, warning=FALSE}
eigTest(eg_full_mat,                        # sample matrices
        cn = full_samples[[1]]$CovRate,     # convergence rate
        cov.arr = eg_full_cov,              # covariance matrices
        testType = 'chi',                   # type of test statistic
        param.out = TRUE)$chi               # return parameters)
...
### Partially common eigenvector estimate

```{r parteigEst, echo=TRUE, message=FALSE, warning=FALSE}
V = expmPartSchur(eg_part_mat,        # sample matrices
...
partialTest(A = eg_part_mat,                    # sample matrices
            cn = part_samples[[1]]$CovRate,     # convergence rate
            cov.arr = eg_part_cov,              # covariance matrices
            k = 3,                              # number of common components
            param.out = TRUE)                   # return parameters
